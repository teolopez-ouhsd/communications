...
Two self-contained tools for keeping Oxnard Union High School District materials consistent:
...
- **`index.html`** — a landing page linking to both, plus a roadmap of what's coming next (school site branding, website content standards, social media guidelines, templates).

Every file is a single, self-contained HTML page — no build step, no server, no external backend. They only reach out to the internet for Google Fonts (Montserrat/Open Sans); everything else (logos, colors, code) is baked into the file itself.

All three pages share the same design system: Material Design–style color roles, a sharper-but-still-rounded shape scale, subtle elevation shadows on cards, and ripple/hover feedback on buttons and chips — built from OUHSD's red and blue.

## Hosting on GitHub Pages

1. Create a new GitHub repository (public, or private if your org's GitHub plan supports Pages on private repos).
2. Upload these three files (`index.html`, `brand-guidelines.html`, `email-signature-generator.html`) to the root of the repository — via the GitHub web UI ("Add file" → "Upload files") or `git push`.
