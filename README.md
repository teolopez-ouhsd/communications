...
- **`website-survey.html`** — a landing page for the district website redesign survey, with separate calls to action for students, parents/guardians, and staff, each linking out to its own Google Form.
...
Every file is a single, self-contained HTML page — no build step, no server, no external backend. They only reach out to the internet for Google Fonts (Montserrat/Open Sans) and, on `website-survey.html`, the linked Google Forms; everything else (logos, colors, code) is baked into the file itself.
...
2. Upload these files (`index.html`, `brand-guidelines.html`, `email-signature-generator.html`, `ui-components.html`, `website-survey.html`) to the root of the repository — via the GitHub web UI ("Add file" → "Upload files") or `git push`.
